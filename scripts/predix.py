...
def deleteExistingApplication(applicationName):
	print("Delete application if it exists: " + applicationName )
	...
	if statementStatus == 1 :
		...
		if statementStatus == 1 :
			print("Error deleting an application: " +deleteRequest)
			sys.exit("Error deleting an application instance: " +serviceName)

	#check if really gone
	if doesItExist("cf a | grep " + applicationName, applicationName, 0) :
...
def deleteExistingService(serviceName):
	print("Delete service if it exists: " +serviceName)
	deleteRequest = "cf delete-service -f " + serviceName
	statementStatus  = subprocess.call(deleteRequest, shell=True)

	if statementStatus == 1 :
		print("Error deleting a service: " +deleteRequest)
		time.sleep(5)  # Delay for 5 seconds
		statementStatus  = subprocess.call(deleteRequest, shell=True)
		if statementStatus == 1 :
			print("Error deleting a service: " +deleteRequest)
			sys.exit("Error deleting a service instance: " +serviceName)

	#check if really gone
	if doesItExist("cf s | grep " + serviceName, serviceName, 0) :
...
def createService(serviceName, serviceRequest):
	...
	if doesItExist("cf s | grep " + serviceName, serviceName, 0) :
		...
		if statementStatus == 1 :
			...
			if not doesItExist("cf s | grep " + serviceName, serviceName, 0) :
				time.sleep(5)
